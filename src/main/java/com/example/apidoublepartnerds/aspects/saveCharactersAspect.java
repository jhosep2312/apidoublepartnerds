...
@Aspect
@Component
public class saveCharactersAspect {

    AuditDB auditDB = new AuditDB();
    final AuditRepository auditRepository;

    public saveCharactersAspect(AuditRepository auditRepository) {
        this.auditRepository = auditRepository;
    }

    @Pointcut("execution(* com.example.apidoublepartnerds.adapter.in.RickAndMortyController.saveCharacters(..))")
    private void saveCharactersAspect() {

    }

    @Before("saveCharactersAspect()  && args(characterName)")
    public void initGetAllCharacters(String characterName) {

        Date date = new Date();
        long time = date.getTime();

        this.auditDB = auditRepository.save(new AuditDB(new Timestamp(time), "GET", characterName, null));
    }

    @AfterReturning(pointcut = "saveCharactersAspect() ", returning = "response")
    public void endGetAllCharacters(ResponseEntity<BaseResponseDto<Characters>> response) {
        log.info("Audit for saveCharactersAspect()  end with code: {}", "response.getBody().getResponseCode()");
